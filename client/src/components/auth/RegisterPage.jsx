...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    setFormErrors({ email: false, password: false, name: false }); // Reset errors
    ...
    if (email && password && name) {
      ...
      try {
        const userData = await register({
          ...
          password,
          name,
          type: data.get('userType') === null ? 0 : 1,
      ...
      >
        ...
        <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 3 }}>
          <Grid container spacing={2}>
            ...
            {!isCoach ? (
              <Grid item xs={6}>
                <TextField fullWidth name="coachID" label="CoachID (optional)" id="coachID" />
              </Grid>
            ) : (
              ''
            )}
